...
read_params() ->
    {ok, Conf} = application:get_env(consume_single, connection_config),
    Conf.

-spec conn_params(map()) -> term(). % @todo fix this typespec
conn_params(Ps) -> 
     Ssl_options = application:get_env(consume_single, ssl_options),
...
validate_conn_name(Name) ->
    ConfigList = application:get_env(consume_single, connection_config, []),
